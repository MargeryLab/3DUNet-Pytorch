...
class DenseVoxelNet(nn.Module):
    ...
    def __init__(self, in_channels=1, classes=1):
        ...
        self.conv_init = nn.Conv3d(1, num_input_features, kernel_size=1, stride=2, padding=0, bias=False)
        ...
        self.conv_final = nn.Sequential(
            nn.Conv3d(self.up_out_features, classes, kernel_size=1, padding=0, bias=False),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, x):   #（2，1，128，128，128）
        # Main network path
        x = self.conv_init(x)   #（2，16，64，64，64）
        x = self.dense_1(x) #（2，160，64，64，64）12个layer的denseblock
        x, t = self.trans(x)    #x是（2，160，32，32,32），t是(2,160,64,64,64)max_pool之前的输出（BN,Relu,1x1）
        x = self.dense_2(x) #（2，304，32，32,32）   共两次下采样
        x = self.up_block(x)    #(2,64,128,128,128)
        y1 = self.conv_final(x) #(2,1,128,128,128)

        # Auxiliary mid-layer prediction, kind of long-skip connection
        t = self.transpose(t)   #(2,64,128,128,128)
        y2 = self.conv_final(t) #(2,1,128,128,128)
        return y1, y2
