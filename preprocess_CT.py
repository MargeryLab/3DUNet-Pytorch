...
class LITS_preprocess:
    ...
    def fix_data(self):
        ...
        for ct_file, i in zip(file_list, range(Numbers)):
            ...
            seg_path = os.path.join(self.raw_root_path, 'label', 'label'+ct_file.split('_')[-1]+'.nii.gz')

            ct_path1 = [file for file in os.listdir(ct_path)][0]
            ct_path2 = [file for file in os.listdir(os.path.join(ct_path, ct_path1))][0]
            ct_path = os.path.join(ct_path, ct_path1, ct_path2)
            new_ct, new_seg = self.process(ct_path, seg_path, classes=self.classes)
            if new_ct != None and new_seg != None:
                sitk.WriteImage(new_ct, os.path.join(self.fixed_path, 'ct', ct_file+'.nii.gz'))
    ...
    def process(self, ct_path, seg_path, classes=None):
        ...
        dcm_path = reader.GetGDCMSeriesFileNames(ct_path)
        ...
        if ct_array.shape[1] == 384 and ct_array.shape[2] == 492:
            self.xy_down_scale = 0.666666666666666666
            ct_array = ndimage.zoom(ct_array, (
                ct.GetSpacing()[-1] / self.slice_down_scale, self.xy_down_scale, self.xy_down_scale), order=3)
            seg_array = ndimage.zoom(seg_array, (
                ct.GetSpacing()[-1] / self.slice_down_scale, self.xy_down_scale, self.xy_down_scale), order=0)
        elif ct_array.shape[1] != ct_array.shape[2]:
            self.xy_down_scale = 0.8
            ct_array = ndimage.zoom(ct_array, (
                ct.GetSpacing()[-1] / self.slice_down_scale, self.xy_down_scale, self.xy_down_scale), order=3)
            seg_array = ndimage.zoom(seg_array, (
                ct.GetSpacing()[-1] / self.slice_down_scale, self.xy_down_scale, self.xy_down_scale), order=0)
        ...
        z = np.any(seg_array, axis=(1, 2))  #np.any(np.array)   对矩阵所有元素做或运算，存在True则返回True,np.all(np.array)   对矩阵所有元素做与操作，所有为True则返回True
    ...
    def write_train_val_name_list(self):
        ...
        assert self.valid_rate < 1.0 and self.test_rate < 1.0
        ...
        test_name_list = data_name_list[
                         int(data_num * (1 - self.test_rate)):int(data_num * ((1 - self.test_rate) + self.test_rate))]
...
if __name__ == '__main__':
    raw_dataset_path = '/media/margery/4ABB9B07DF30B9DB/MedicalImagingDataset/Pancreas-CT'
    fixed_dataset_path = '/media/margery/4ABB9B07DF30B9DB/MedicalImagingDataset/Pancreas-CT/fixed_data'
